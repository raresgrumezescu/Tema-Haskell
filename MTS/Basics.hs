...
data Gateway = Gateway { thisPos :: Position
		, twinPos :: Position
} deriving (Eq, Ord)

data Game = Game {hunterPos :: Position
		, line :: Int
		, columns :: Int
		, harta :: [Char]
} deriving (Eq, Ord)
...
{-
makeEmptyLine :: Int -> String -> String
makeEmptyLine 1 ch = ch
makeEmptyline c ch = " " ++ makeEmptyLine (c - 1) ch

makeLine :: Int -> String
makeLine 1 = "@"
makeLine c = "@" ++ makeLine (c - 1)

helpGameAsString :: Game -> Int -> Int -> String
helpGameAsString game curr c = if curr == c then makeLine c
				else if curr == 1 then makeLine c ++ "\n" ++ helpGameAsString game (curr + 1) c
					else "@" ++ makeEmptyLine (c - 1) "@" ++ "\n" ++ helpGameAsString game (curr + 1) c




gameAsString :: Game -> String
gameAsString game = helpGameAsString game 1 (columns game)
-}

gameAsString :: Game -> String
gameAsString game = (harta game)
...
makeEmptyBoard :: Int -> Int -> Int -> [Char]
makeEmptyBoard l c dim
    | l == 0 = replicate (c) '@' ++ ['\n'] ++ (makeEmptyBoard (l + 1) c dim)
    | l == dim = replicate (c) '@'
    | l == 1 = ['@'] ++ ['!'] ++ replicate (c - 3) ' ' ++ ['@'] ++ ['\n'] ++ (makeEmptyBoard (l + 1) c dim)
    | otherwise = ['@'] ++ replicate (c - 2) ' ' ++ ['@'] ++ ['\n'] ++ (makeEmptyBoard (l + 1) c dim)

pH :: Position
pH = (1, 1)

emptyGame :: Int -> Int -> Game
emptyGame linii coloane = Game pH linii coloane (makeEmptyBoard 0 coloane (linii - 1))
...
isGoodHere :: Position -> [Char] -> Int -> Int -> Bool
{-((fst pos) * (c + 1) + (snd pos) > -1) && ((fst pos) * (c + 1) + (snd pos) < (c + 1) * l + 1) == True-}
isGoodHere pos harta c l = maybe False id (if ((fst pos) < 0 || (snd pos) < 0 || (fst pos) > (l - 1) || (snd pos) > (c - 1)) == False then if last (take ((fst pos) * (c + 1) + (snd pos) + 1) harta) == ' ' then (Just True)
															else Nothing
				else Nothing)

deleteHunter :: [Char] -> Position -> Int -> [Char]
deleteHunter harta pos c = if (last (take ((fst pos) * (c + 1) + (snd pos) + 1) harta)) == '!' then take ((fst pos) * (c + 1) + (snd pos)) harta ++ [' '] ++ drop ((fst pos) * (c + 1) + (snd pos) + 1) harta
				else harta

replaceHunter :: [Char] -> Position -> Int -> [Char]
replaceHunter harta pos c = take ((fst pos) * (c + 1) + (snd pos)) harta ++ ['!'] ++ drop ((fst pos) * (c + 1) + (snd pos) + 1) harta
				
addHunter :: Position -> Game -> Game
addHunter pos game = if (isGoodHere pos (harta game) (columns game) (line game)) then (Game (hunterPos game) (line game) (columns game) (replaceHunter (deleteHunter (harta game) (hunterPos game) (columns game)) pos (columns game)))
			else game
...
putTarget :: [Char] -> Position -> Int -> [Char]
putTarget harta pos c = take ((fst pos) * (c + 1) + (snd pos)) harta ++ ['*'] ++ drop ((fst pos) * (c + 1) + (snd pos) + 1) harta

addTarget :: Behavior -> Position -> Game -> Game
addTarget  beh pos game = if (isGoodHere pos (harta game) (columns game) (line game)) then (Game (hunterPos game) (line game) (columns game) (putTarget (harta game) pos (columns game)))
				else game
...
isGoodHere2 :: Position -> [Char] -> Int -> Int -> Bool
{-((fst pos) * (c + 1) + (snd pos) > -1) && ((fst pos) * (c + 1) + (snd pos) < (c + 1) * l + 1) == True-}
isGoodHere2 pos harta c l = maybe False id (if ((fst pos) < 0 || (snd pos) < 0 || (fst pos) > (l - 1) || (snd pos) > (c - 1)) == False then if ((last (take ((fst pos) * (c + 1) + (snd pos) + 1) harta) == ' ') || (last (take ((fst pos) * (c + 1) + (snd pos) + 1) harta) == '!')) == True then (Just True)
															else Nothing
				else Nothing)

putGateway :: [Char] -> Position -> Int -> [Char]
putGateway harta pos c = take ((fst pos) * (c + 1) + (snd pos)) harta ++ ['#'] ++ drop ((fst pos) * (c + 1) + (snd pos) + 1) harta

addGateway :: (Position, Position) -> Game -> Game
addGateway gatePosis game = (Game (hunterPos game) (line game) (columns game) (putGateway (putGateway (harta game) (fst gatePosis) (columns game)) (snd gatePosis) (columns game)))
 {-if (isGoodHere (fst gatePosis) (harta game) (columns game) (line game)) && (isGoodHere (snd gatePosis) (harta game) (columns game) (line game)) then-}
{-
    *** TODO ***
...
putObstacle :: [Char] -> Position -> Int -> [Char]
putObstacle harta pos c = take ((fst pos) * (c + 1) + (snd pos)) harta ++ ['@'] ++ drop ((fst pos) * (c + 1) + (snd pos) + 1) harta

addObstacle :: Position -> Game -> Game
addObstacle pos game = if (isGoodHere pos (harta game) (columns game) (line game)) then (Game (hunterPos game) (line game) (columns game) (putObstacle (harta game) pos (columns game)))
				else game
...
gatewayVecin :: Position -> [Gateway] -> Position
gatewayVecin pos gates = if thisPos (head gates) == pos then (2, 2) {-(thisPos (head gates))-}
			else (0, 0){-(gatewayVecin pos (tail gates))-}

attemptMove :: Position -> Game -> Maybe Position

{-
attemptMove pos game = if (last (take ((fst pos) * ((columns game) + 1) + (snd pos) + 1) (harta game))) == ' ' then (Just (0, 0))
			else if (last (take ((fst pos) * ((columns game) + 1) + (snd pos) + 1) (harta game))) == '#' then (Just (5, 5)){-(Just (gatewayVecin pos (listOfGateways game)))-}
				else if last (take ((fst pos) * ((columns game) + 1) + (snd pos) + 1) (harta game)) == '@' then Nothing
					else Nothing
-}

attemptMove pos game = undefined
{-
    *** TODO ***
